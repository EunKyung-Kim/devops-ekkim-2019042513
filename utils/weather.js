...
const m_weather_endpoint = {
	...
	almanac_monthly: '/almanac/monthly.json?start=' + moment(new Date()).add(1,'days').format('MM'),
...
const m_weather_input = {
	...
	url: m_weather_url,
	headers: {
		Authorization: 'Basic ' + new Buffer.from(m_weather_cred.username + ':' + m_weather_cred.password).toString('base64')
	}
}

//function putWeatherToDB() {
var putWeatherToDB = () => new Promise((resolve,reject)=>{
	var m_output = {};
		//20년 데이터 호출
	get_almanac()
	.then(m_result => {
	...
	.then(results => {
		//DB에 적재
		var m_query = "INSERT INTO t_weather " +
...
//20년간 통계
var get_almanac = () => new Promise((resolve,reject)=>{
...
//내일 날씨 예측
var get_forecast = () => new Promise((resolve,reject)=>{
...
//현재 날씨
var get_current = () => new Promise((resolve,reject)=>{
...
var get_history = () => new Promise((resolve,reject)=>{
...
var dbquery = (i_query, i_params) => new Promise((resolve,reject)=>{
	//db.connect();
